...
### 💎 2.7 Максимизация выражения: `sum(A[i]) * min(B[i])` для подмножества размера k

**Пример задачи**: [2542. Maximum Subsequence Score](https://leetcode.com/problems/maximum-subsequence-score)

---

### 📌 Когда использовать:

* Нужно выбрать **ровно k элементов**
* Выражение: **сумма по одному массиву, минимум по другому**
* Ищем **максимальное значение формулы** `sum(nums1[i]) * min(nums2[i])`

---

### 🧠 Подход:

1. Объединяем `nums2[i]` и `nums1[i]` в пары: `[nums2[i], nums1[i]]`
2. Сортируем пары по убыванию `nums2[i]`

   * Это значит: мы "фиксируем" `min(nums2)` на каждом шаге
3. Используем **MinHeap**, чтобы выбрать `k` наибольших `nums1[i]`

   * `heap` хранит текущие `k` лучших `nums1[i]`, поддерживаем их сумму
4. На каждом шаге, если в куче уже `k` элементов:

   * Вычисляем `score = sum(nums1) * current nums2[i]`
   * Обновляем максимум
...
```go
type MinHeap []int
// реализация heap (Len, Less, Swap, Push, Pop)

sort.Sort(pairs by nums2 ↓)
heap := пустой min-heap
sum := 0
...
**Фиксируем текущий минимум из `nums2`, и к нему подбираем `k` наибольших `nums1`, чтобы максимизировать выражение.**
